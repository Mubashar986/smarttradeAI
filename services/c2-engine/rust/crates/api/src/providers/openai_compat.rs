...
const GROQ_RESET_REQUESTS_HEADER: &str = "x-ratelimit-reset-requests";
...
pub struct OpenAiCompatConfig {
    ...
    pub fallback_api_key_envs: &'static [&'static str],
    pub fallback_base_url_envs: &'static [&'static str],
}
...
impl OpenAiCompatConfig {
    ...
    pub const fn openai() -> Self {
        Self {
            ...
            fallback_api_key_envs: &[],
            fallback_base_url_envs: &["OPENAI_API_BASE"],
        }
    }
    ...
    pub const fn groq() -> Self {
        Self {
            ...
            fallback_base_url_envs: &["OPENAI_BASE_URL", "OPENAI_API_BASE"],
        }
    }
...
impl OpenAiCompatClient {
    ...
    pub fn from_env(config: OpenAiCompatConfig) -> Result<Self, ApiError> {
        let Some(api_key) = read_api_key(config)? else {
    ...
    async fn send_with_retry(
        ...
        let last_error = loop {
            ...
            tokio::time::sleep(self.retry_delay_for_error(&retryable_error, attempts)?).await;
        };
            ...
            events.push(StreamEvent::MessageStart(MessageStartEvent {
                message: MessageResponse {
                    id: chunk
                        .id
                        .clone()
                        .if_empty_then("openai_compat_stream".to_string()),
        ...
        _ => message
            ...
            .filter_map(|block| match block {
                ...
                InputContentBlock::ToolResult {
                    ...
                    "content": format_tool_result_content(content, *is_error),
                })),
...
fn format_tool_result_content(content: &[ToolResultContentBlock], is_error: bool) -> String {
...
fn normalize_response(
    ...
    Ok(MessageResponse {
        id: response
            .id
            .if_empty_then("openai_compat_message".to_string()),
        kind: "message".to_string(),
...
fn read_api_key(config: OpenAiCompatConfig) -> Result<Option<String>, ApiError> {
    let mut keys = vec![config.api_key_env];
    keys.extend(config.fallback_api_key_envs.iter().copied());
...
pub fn read_base_url(config: OpenAiCompatConfig) -> String {
    for key in std::iter::once(config.base_url_env).chain(config.fallback_base_url_envs.iter().copied()) {
...
async fn expect_success(response: reqwest::Response) -> Result<reqwest::Response, ApiError> {
    ...
    let retry_after = retry_after_from_response(
        status,
        &headers,
        parsed_error.as_ref().and_then(|error| error.error.message.as_deref()),
        &body,
    );
    let retryable = is_retryable_status(status, retry_after.is_some());
...
fn is_retryable_status(status: reqwest::StatusCode, has_retry_after: bool) -> bool {
    matches!(status.as_u16(), 408 | 409 | 500 | 502 | 503 | 504)
        || (status == reqwest::StatusCode::TOO_MANY_REQUESTS && has_retry_after)
}
...
fn retry_after_from_response(
    ...
    for header_name in [
        ...
        GROQ_RESET_TOKENS_HEADER,
        GROQ_RESET_REQUESTS_HEADER,
    ] {
        if let Some(value) = headers.get(header_name).and_then(|value| value.to_str().ok()) {
            if let Some(delay) = parse_retry_after_hint(value) {
                return Some(delay);
            }
        }
    }
    ...
    message
        .and_then(extract_retry_after_from_text)
        .or_else(|| extract_retry_after_from_text(body))
}

fn extract_retry_after_from_text(text: &str) -> Option<Duration> {
    ...
    for marker in ["try again in ", "retry after "] {
        if let Some(start) = lower.find(marker) {
            ...
            let token = suffix
                ...
                .collect::<String>();
            if let Some(delay) = parse_retry_after_hint(&token) {
                return Some(delay);
            }
        }
    }
...
fn parse_retry_after_hint(raw: &str) -> Option<Duration> {
    ...
    if let Some(stripped) = trimmed.strip_suffix('s') {
        if let Some((minutes, seconds)) = stripped.split_once('m') {
            let minutes = minutes.trim().parse::<f64>().ok()?;
            let seconds = seconds.trim().parse::<f64>().ok()?;
            return duration_from_seconds((minutes * 60.0) + seconds);
        }

        if let Ok(seconds) = stripped.trim().parse::<f64>() {
            return duration_from_seconds(seconds);
        }
    }
...
fn duration_from_seconds(seconds: f64) -> Option<Duration> {
    ...
    (duration <= MAX_RATE_LIMIT_RETRY_DELAY).then_some(duration)
}
...
mod tests {
    use super::{
        ...
        openai_tool_choice, parse_retry_after_hint, parse_tool_arguments, OpenAiCompatClient,
        OpenAiCompatConfig,
    };
    use crate::error::ApiError;
    use crate::types::{
    ...
    #[test]
    fn tool_error_content_is_encoded_without_is_error_field() {
        let payload = build_chat_completion_request(&MessageRequest {
            model: "grok-3".to_string(),
            max_tokens: 64,
            messages: vec![InputMessage {
    ...
    fn parses_retry_after_hints() {
        assert_eq!(
            parse_retry_after_hint("11.195s"),
            Some(Duration::from_secs_f64(11.195))
        );
        assert_eq!(parse_retry_after_hint("7"), Some(Duration::from_secs(7)));
        assert_eq!(parse_retry_after_hint("25s"), Some(Duration::from_secs(25)));
        assert_eq!(parse_retry_after_hint("65s"), None);
    }

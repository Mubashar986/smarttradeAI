...
// Inlined from deleted sandbox.rs — only the types config.rs needs
...
fn read_optional_json_object(path: &Path) -> Result<Option<JsonObject>, ConfigError> {
...
fn expect_object<'a>(
    value: &'a JsonValue,
    context: &str,
) -> Result<&'a JsonObject, ConfigError> {
    value
        .as_object()
...
fn deep_merge_objects(target: &mut JsonObject, source: &JsonObject) {
...
mod tests {
    use super::{
        ConfigLoader, ConfigSource, FilesystemIsolationMode, McpServerConfig, McpTransport,
